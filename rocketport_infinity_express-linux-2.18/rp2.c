 ...
 *
 * Updates and modifications: Copyright (C) Comtrol Corp.
 *
...
# define rp2_tty_is_low_latency(tty)                  tty->low_latency
...
// Compatiblity macros for timer API change
...
# define timer_setup(timer, callback, flags) setup_timer((timer), (callback), (unsigned long)(timer))
...
} port_mode_tbl[] =
{
  ...
  {"rs232h",   RP2_UART_CTL_MODE_rs232,  RP2_TXRX_CTL_RTS_TOGGLE_m, 0,                           "rs232 half-duplex"},
  {"rs422",    RP2_UART_CTL_MODE_rs422,  0,                         0,                           "rs422"},
  {"rs485",    RP2_UART_CTL_MODE_rs485,  RP2_TXRX_CTL_RTS_TOGGLE_m, RP2_TXRX_CTL_RTS_POLARITY_m, "rs485 2-wire"},
  {"rs485-2",  RP2_UART_CTL_MODE_rs485,  RP2_TXRX_CTL_RTS_TOGGLE_m, RP2_TXRX_CTL_RTS_POLARITY_m, "rs485 2-wire"},
  {"rs485-4s", RP2_UART_CTL_MODE_rs485s, RP2_TXRX_CTL_RTS_TOGGLE_m, RP2_TXRX_CTL_RTS_POLARITY_m, "rs485 4-wire slave"},
...
{
  rp2_rmw(up, RP2_TXRX_CTL, RP2_TXRX_CTL_RX_TRIG_m, low_latency ? RP2_TXRX_CTL_RX_TRIG_1 : RP2_TXRX_CTL_RX_TRIG_256);
}
...
static void rp2_uart_unthrottle(struct uart_port *port)
{
  ...
  up->throttled = 0;
  enable_rx_fifo_trigger(up,rp2_tty_is_low_latency(tty));
}
...
static void rp2_rx_chars(struct rp2_uart_port *up)
{
  ...
  if (chanstat & RP2_CHAN_STAT_RXERR_m)
    {
      ...
      rp2_tty_insert_flip_string_flags(tty,up->rxbuf.u8,up->fbuf,bytes);
    }
  else
    {
  ...
  // bytes
  portMonitorQueueBytes(up->monp, 0, up->rxbuf.u8, bytes, polling_mode == 0);
...
static void rp2_tx_chars(struct rp2_uart_port *up)
{
  ...
  if (txcount)
    portMonitorQueueBytes(up->monp, 1, up->txbuf, txcount, 1);
}
  ...
  if (status & RP2_CHAN_STAT_MS_CHANGED_MASK)
    {
      ...
      if (status & RP2_CHAN_STAT_CD_CHANGED_m)
        uart_handle_dcd_change(&up->port, status & RP2_CHAN_STAT_DCD_m);
      if (status & RP2_CHAN_STAT_CTS_CHANGED_m)
        uart_handle_cts_change(&up->port, status & RP2_CHAN_STAT_CTS_m);
...
{
  rp2_rmw_set(up, RP2_UART_CTL, RP2_UART_CTL_FLUSH_RX_m | RP2_UART_CTL_FLUSH_TX_m);
  readl(up->base + RP2_UART_CTL);
  udelay(10);
  rp2_rmw_clr(up, RP2_UART_CTL, RP2_UART_CTL_FLUSH_RX_m | RP2_UART_CTL_FLUSH_TX_m);
}
...
static int rp2_uart_startup(struct uart_port *port)
{
  ...
  tty = rp2_tty(up);
  rp2_flush_fifos(up);
  rp2_rmw(up, RP2_TXRX_CTL, 0, RP2_TXRX_CTL_MSRIRQ_m | RP2_TXRX_CTL_RXIRQ_m | RP2_TXRX_CTL_RX_TMOUTIRQ_m);
  rp2_rmw(up, RP2_TXRX_CTL, RP2_TXRX_CTL_TX_TRIG_m, RP2_TXRX_CTL_TX_TRIG_128);
  enable_rx_fifo_trigger(up, rp2_tty_is_low_latency(tty));
...
static void rp2_init_port(struct rp2_uart_port *up, const struct firmware *fw)
{
  ...
  writel((RP2_UART_CTL_FLUSH_RX_m | RP2_UART_CTL_FLUSH_TX_m), up->base + RP2_UART_CTL);
  readl(up->base + RP2_UART_CTL);
...
{
  int i;
  struct rp2_card *card = from_timer(card, param, poll_timer);
...
static int rp2_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
  ...
  if (card->minor_start < 0)
    {
      dev_err(&pdev->dev, "too many ports (RP2_NR_UARTS in rp2.c is " xstr(RP2_NR_UARTS) ")\n");
      return -EINVAL;
    }

  rp2_init_card(card);

  ports = devm_kzalloc(&pdev->dev, sizeof(*ports) * card->n_ports, GFP_KERNEL);
  if (!ports)
    return -ENOMEM;
  card->ports = ports;
...
#endif
{
  ...
  { RP_ID(0x0047), RP_CAP(4,  0) }, /* INF 4, RJ45 */
  { RP_ID(0x0048), RP_CAP(8,  0) }, /* INF 8, RJ11 */
  ...
  { RP_ID(0x006f), RP_CAP(2,  0) }, /* EXP SMPTE */
...
static void __init parse_port_modes(char *modes[], int count)
{
  int i;
  for (i=0; i<count; ++i)
    {
      ...
      if (3 == sscanf(mode,"%u-%u:%14s",&start_port,&end_port,modestr))
        {
        }
      else if (2 == sscanf(mode,"%u:%14s",&start_port,modestr))
        {
          end_port = start_port;
        }
      else
        {
          printk(KERN_WARNING "rp2: unrecognized mode setting: '%s'\n",mode);
          continue;
        }

      if (DEBUG_PORT_MODES > 1)
        printk(KERN_DEBUG "rp2: start_port=%u  end_port=%u  modestr='%s'\n",start_port,end_port,modestr);

      if (start_port >= MaxPorts)
        {
          printk(KERN_WARNING "rp2: illegal port number %u in setting '%s'\n",start_port,mode);
          continue;
        }
      ...
      if (!p->abbrev)
        {
          printk(KERN_WARNING "rp2: unrecognized port mode '%s' for port %u\n",modestr,start_port);
          continue;
        }
      ...
      for (j=start_port; j<=end_port; ++j)
        {
          port_mode[j] = p;
          if (DEBUG_PORT_MODES)
            printk(KERN_DEBUG "rp2: port %u mode: %s\n",j,p->modestr);
        }
    }
}
  ...
  if (polling_mode)
    {
      if (polling_mode < 10)
        {
          printk(KERN_WARNING "rp2: Polling frequency (%dHz) is too low -- using %dHz\n",polling_mode,10);
          polling_mode = 10;
        }
      polling_jiffies = HZ/polling_mode;
      if (polling_jiffies == 0)
        {
          printk(KERN_WARNING "rp2: Polling frequency (%dHz) is too high -- using %dHz\n",polling_mode,HZ);
          polling_jiffies = 1;
        }
    }

  parse_port_modes(port_modes, port_modes_count);
...
MODULE_PARM_DESC(port_modes,"Port interface settings. Example: port_modes=0:232,1:485,2-7:232,8-15:422 ");
module_param(polling_mode,int,0444);
MODULE_PARM_DESC(polling_mode, "Polling frequency in Hz (0 == interrupt-driven)");
...
MODULE_AUTHOR("Comtrol Corp. <support@comtrol.com>");
